...
import MonitoringLogForm from '../../../components/maintenance/MonitoringLogForm';
            ...
            {tab === 'monitoring' && (
                <div className="space-y-6">
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <button onClick={() => { setSelectedLog(null); setShowLogForm(true); }} className="flex items-center gap-1 rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-white">
                    ...
                    {showLogForm && (
                        <MonitoringLogForm
                            ...
                            equipmentId={id!}
                            config={config}
                            onClose={() => { setShowLogForm(false); setSelectedLog(null); }}
                            onSaved={fetchAll}
                        />
                    )}
                        ...
                        <div className="overflow-x-auto rounded-xl border border-slate-200/60 bg-white/50 shadow-sm dark:border-slate-800/60 dark:bg-slate-900/50">
                            <table className="w-full text-xs">
                                ...
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                    {logs.map(log => (
                                        <tr key={log.id} 
                                            onClick={() => { setSelectedLog(log); setShowLogForm(true); }}
                                            className="cursor-pointer hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                            <td className="px-3 py-2 font-medium text-slate-900 dark:text-white">{new Date(log.log_date + 'T00:00:00').toLocaleDateString('es-MX')}</td>
                            ...
                            {logs.filter(l => l.photos && l.photos.length > 0).slice(0, 5).map(log => (
                                <div key={log.id} className="rounded-lg border border-slate-200/60 p-3 dark:border-slate-700/60">
                                    <p className="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-2">
                                        {new Date(log.log_date + 'T00:00:00').toLocaleDateString('es-MX')} — {log.recorded_by || 'Sin autor'}
                                    </p>
                    ...
                    {showVideoForm && (
                        <form onSubmit={handleAddVideo} className="rounded-xl border border-primary/20 bg-primary/5 p-6">
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                                ...
                                <div><label className={labelClass}>Material Ademe</label><input type="text" value={videoForm.ademe_material} onChange={e => setVideoForm({ ...videoForm, ademe_material: e.target.value })} className={inputClass} /></div>
                                <div><label className={labelClass}>Diám. Ademe</label><input type="text" value={videoForm.ademe_diameter} onChange={e => setVideoForm({ ...videoForm, ademe_diameter: e.target.value })} className={inputClass} /></div>
                                <div><label className={labelClass}>Tipo Ranuras</label><input type="text" value={videoForm.slot_type} onChange={e => setVideoForm({ ...videoForm, slot_type: e.target.value })} className={inputClass} /></div>
                                ...
                                <div className="md:col-span-4"><label className={labelClass}>URL del Video</label><input type="url" value={videoForm.video_url} onChange={e => setVideoForm({ ...videoForm, video_url: e.target.value })} className={inputClass} placeholder="https://youtube.com/..." /></div>
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-bold text-sm text-slate-900 dark:text-white">
                                                        {new Date(log.log_date + 'T00:00:00').toLocaleDateString('es-MX', { weekday: 'short', day: 'numeric', month: 'long', year: 'numeric' })}
