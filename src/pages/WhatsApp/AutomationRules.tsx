...
export default function AutomationRules() {
    ...
    const fetchAll = useCallback(async () => {
        ...
        const [rRes, tRes, uRes] = await Promise.all([
            ...
            supabase.from('wa_templates').select('*').eq('meta_status', 'approved').order('name'),
                ...
                <div className="px-6 pt-6 pb-0">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-xl font-bold text-slate-900 dark:text-white">Automatizaciones WhatsApp</h1>
                                <p className="text-xs text-slate-500">Notificaciones automáticas al equipo cuando ocurre un evento</p>
                            </div>
                        </div>
                        <button onClick={() => { resetForm(); setShowForm(true); }}
                            className="flex items-center gap-2 rounded-lg bg-emerald-500 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-emerald-500/25 hover:bg-emerald-600">
                            <span className="material-symbols-outlined text-[18px]">add</span>Nueva Regla
                        </button>
                    </div>
                    <div className="flex gap-1 overflow-x-auto">
                        {tabs.map(t => (
                            <button key={t.path} onClick={() => navigate(t.path)}
                                className={`flex items-center gap-1.5 rounded-t-lg px-4 py-2.5 text-xs font-semibold transition-all whitespace-nowrap ${t.path === '/whatsapp/automations'
