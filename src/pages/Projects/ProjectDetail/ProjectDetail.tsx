        ...
        if (newStatus === 'in_field' && !project.actual_start) {
            ...
            if (!projectSpaceId) {
                const { data: newSpace } = await supabase.from('spaces').insert({
                    ...
                    members: ['Joel', 'Ruby', 'Administración', 'Operaciones', 'Ingeniería'],
                    is_archived: false,
                }).select().single();
            ...
            const checklistItems = [
                { field: 'checklist_invoice', title: `Factura previa — ${project.project_number}`, assignee: 'Facturación y Cobranza' },
                { field: 'checklist_materials', title: `Materiales listos — ${project.project_number}`, assignee: 'Almacén y Compras' },
                { field: 'checklist_vehicle', title: `Vehículo preparado — ${project.project_number}`, assignee: 'Operaciones' },
                { field: 'checklist_team', title: `Equipo asignado — ${project.project_number}`, assignee: 'Ingeniería' },
            ];
                        ...
                        <div className={sectionClass}>
                            ...
                            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                {([
                                    ...
                                    { field: 'checklist_team' as ChecklistField, label: 'Trabajo Programado', icon: 'group', responsible: 'Ingeniería / Operaciones' },
                                ]).map(item => {
