...
/* ───── Component ───── */
export default function MonthlyExitsReport() {
    ...
    /* ─── Fetch exits for selected month ─── */
    ...
    // KPIs
    const totalExits = movements.length;
    ...
    const avgCostPerExit = totalExits > 0 ? totalCost / totalExits : 0;
    ...
    const dailyTrend = useMemo(() => {
        ...
        const days: { day: string; salidas: number; costo: number }[] = [];
        for (let d = 1; d <= daysInMonth; d++) {
            days.push({ day: d.toString(), salidas: 0, costo: 0 });
        }
        movements.forEach(m => {
            const date = new Date(m.created_at);
            const dayIndex = date.getDate() - 1;
            if (dayIndex >= 0 && dayIndex < days.length) {
                days[dayIndex].salidas += 1;
    ...
    // 4. Distribución por razón de salida
    ...
    const detailedRows = useMemo(() => {
        const map = new Map<string, {
            ...
            totalQty: number; totalCost: number; exits: number;
        }>();
        movements.forEach(m => {
            const id = m.product_id;
            const cost = m.total_cost || m.quantity * (m.unit_cost || 0);
    ...
    return (
        <div className="flex flex-1 flex-col gap-6 p-8">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500">
                            Análisis de materiales con salida — <strong className="text-indigo-500">{MONTH_NAMES[month]} {year}</strong>
                        </p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {[
                    { label: 'Salidas Totales', value: totalExits.toString(), icon: 'output', color: 'from-indigo-500 to-violet-500' },
                    { label: 'Costo Total Salidas', value: formatCurrencyInv(totalCost), icon: 'payments', color: 'from-red-500 to-rose-500' },
                    { label: 'Materiales Distintos', value: uniqueProducts.toString(), icon: 'inventory_2', color: 'from-amber-500 to-orange-500' },
                    { label: 'Costo Prom. / Salida', value: formatCurrencyInv(avgCostPerExit), icon: 'analytics', color: 'from-emerald-500 to-teal-500' },
                ].map(k => (
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center gap-3 py-16">
                    ...
                    <p className="text-sm text-slate-500">Sin salidas de materiales registradas en {MONTH_NAMES[month]} {year}.</p>
                    <p className="text-xs text-slate-400">Selecciona otro mes o registra salidas desde el inventario.</p>
                </div>
            ) : (
                <>
                        ...
                        <div className={`${sectionClass} p-5 lg:col-span-2`}>
                            ...
                            <ResponsiveContainer width="100%" height={260}>
                                <AreaChart data={dailyTrend} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                                    ...
                                    <Area yAxisId="left" type="monotone" dataKey="salidas" name="Salidas" stroke="#f43f5e" fill="url(#gradSalidas)" strokeWidth={2} />
                        ...
                        <div className={`${sectionClass} p-5`}>
                                            ...
                                            <div key={r.name}>
                                                <div className="flex items-center justify-between mb-1">
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        <span className="text-xs text-slate-400">{r.count} salidas</span>
                            ...
                            <div className="mt-6 rounded-lg border border-indigo-100 bg-indigo-50/50 p-4 dark:border-indigo-900/40 dark:bg-indigo-900/10">
                                ...
                                <ul className="space-y-1 text-xs text-slate-600 dark:text-slate-400">
                                    ...
                                    {dailyTrend.length > 0 && (() => {
                                        const peakDay = dailyTrend.reduce((max, d) => d.salidas > max.salidas ? d : max, dailyTrend[0]);
                                        return peakDay.salidas > 0 ? (
                                            <li>• El día con más actividad fue el <strong className="text-slate-900 dark:text-white">{peakDay.day} de {MONTH_NAMES[month]}</strong> con {peakDay.salidas} salidas.</li>
                                        ) : null;
                                    })()}
                                    <li>• Promedio diario: <strong className="text-slate-900 dark:text-white">{(totalExits / new Date(year, month + 1, 0).getDate()).toFixed(1)}</strong> salidas/día.</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-slate-100 dark:border-slate-800">
                                        ...
                                        <th className="pb-3 text-right text-xs font-semibold text-slate-400">Salidas</th>
                                ...
                                <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
                                    {detailedRows.map((row, idx) => (
                                        <tr key={row.product_id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                            ...
                                            <td className="py-2.5 text-right text-slate-600 dark:text-slate-300 font-semibold">{row.exits}</td>
                                ...
                                <tfoot>
                                    <tr className="border-t-2 border-slate-200 dark:border-slate-700">
                                        ...
                                        <td className="py-3 text-right font-bold text-slate-900 dark:text-white">{totalExits}</td>
